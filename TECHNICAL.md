...
0. **Fast-path liveness probe**: `GET /healthz` is mounted before the rest of the stack and returns HTTP 204 with an empty body. This keeps orchestrator probes cheap and bypasses cookie/session/CSRF/settings middleware.
...
5. **`express-session`** issues the session cookie, stamped `httpOnly`, `sameSite=lax`, `secure` in production, with a configurable TTL (`SESSION_TTL_SECONDS`, default 28 800 s = 8 h). The store is `session-file-store` writing one JSON-per-sid into `data/sessions/`, reaped every 60 minutes. Sessions therefore survive process restart and rolling deploys — see §6.5.
6. **`csrf-csrf` double-submit cookie** issues a consistent `_csrf-token` cookie in all environments and binds the token to the current `session.id`. A custom getter accepts the matching token from either the `_csrf` form field or the `x-csrf-token` request header.
...
10. **Locals propagation + request-scoped settings** sets `res.locals.user`, `res.locals.isPortalAdmin`, `res.locals.assetVersion`, `res.locals.csrfToken`, and branding flags. It also reads settings once per request and stores them in `res.locals.settings` for downstream route reuse.
